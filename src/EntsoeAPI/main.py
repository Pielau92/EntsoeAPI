...
today = session.date_today  # today's date
...
if datetime.datetime.now().time() < datetime.datetime.strptime(session.configs.general.day_ahead_deadline,
                                                               '%H:%M').time():
    print(f'Day ahead prognosis data not available until today {session.configs.general.day_ahead_deadline}.')
else:
    params.append(('forecast', 'tomorrow'))

# add yearly historical data requests
[params.append(('historical', year)) for year in session.configs.general.years]

# add yearly generation by energy source data requests
[params.append(('generation_by_source', year)) for year in session.configs.general.years]

# add yearly generation by energy source data requests
[params.append(('imports', year)) for year in session.configs.general.years]

# endregion

# perform API requests
datasets = {f'{query_name}_{tp}': get_query(data_query=session, tp=tp, query_name=query_name) for query_name, tp in
            params}
